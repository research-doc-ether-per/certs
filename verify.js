...
class SDJwtVC(sdJwt: SDJwt): SDJwt(sdJwt.jwt, sdJwt.header, sdJwt.sdPayload, sdJwt.keyBindingJwt, sdJwt.isPresentation) {
  ...
  init {
    println(
        ...
        issuer      = $issuer
        holderDid   = $holderDid
        holderJWK   = ${holderKeyJWK?.get("kid")?.jsonPrimitive?.content ?: "null"}
        notBefore   = $notBefore
  ...
  private fun verifyHolderKeyBinding(jwtCryptoProvider: JWTCryptoProvider, requiresHolderKeyBinding: Boolean,
    ...
    return if(!holderDid.isNullOrEmpty()) {
      // TODO("Holder DID verification not yet supported")
      val holderKeyJson = keyBindingJwt?.jwt?.let { getHolderKeyJWKFromKbJwtImpl(it) }
          ?: throw UnsupportedOperationException("Unable to resolve holder key: no cnf.jwk found in kb_jwt")

      val holderKid = holderKeyJson["kid"]?.jsonPrimitive?.content
          ?: throw IllegalArgumentException("holderKeyJson does not contain 'kid'")

      println(
          """
          ── Extracted Holder Key ────────────────
          holderKeyJson = $holderKeyJson
          holderKid     = $holderKid
          ───────────────────────────────────────
          """.trimIndent()
      )
      ...
      isPresentation && keyBindingJwt != null && !audience.isNullOrEmpty() && !nonce.isNullOrEmpty() &&
          keyBindingJwt.verifyKB(jwtCryptoProvider, audience, nonce, this, holderKid)
